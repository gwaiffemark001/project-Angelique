...
def detect_choch(candles: list[dict[str, Any]], liquidity_swept: bool, htf_bias: str | None = None) -> dict[str, Any]:
    """Validate CHOCH as a contextual reversal, not an isolated structure break."""
    closed = _closed_candles(candles)
    if len(closed) < 10:
        return {"valid": False, "reason": "insufficient_data"}
    last = closed[-1]
    recent = closed[-11:-1]
    prior_high = max((_high(item) for item in recent), default=0.0)
    prior_low = min((_low(item) for item in recent), default=0.0)
    candle_range = max(_high(last) - _low(last), 1e-12)
    body_ratio = abs(_close(last) - _open(last)) / candle_range
    direction = "bullish" if _close(last) > prior_high else "bearish" if _close(last) < prior_low else None
    if not direction:
        return {"valid": False, "reason": "no_structure_break", "body_ratio": body_ratio}
    expected = str(htf_bias or "").strip().lower()
    if not liquidity_swept:
        return {"valid": False, "reason": "liquidity_sweep_required", "direction": direction, "body_ratio": body_ratio}
    if body_ratio < 0.60:
        return {"valid": False, "reason": "weak_break_body", "direction": direction, "body_ratio": body_ratio}
    if expected in {"bullish", "bearish"} and expected != direction:
        return {"valid": False, "reason": "htf_bias_conflict", "direction": direction, "htf_bias": expected, "body_ratio": body_ratio}
    return {"valid": True, "direction": direction, "htf_bias": expected or None, "body_ratio": body_ratio, "reason": "sweep_then_strong_body_break"}
...
def detect_smc(candles: list[dict[str, Any]], direction: str | None = None, timeframe: str | None = None, registry: ZoneRegistry | None = None) -> dict[str, Any]:
    ...
    strict_choch = detect_choch(candles, liquidity_swept=bool(sweep))
    ...
    # Canonical FVG execution playbook: precise swing-break association, chop
    # filtering, retest/hold, 50% midpoint entries, IFVG inversion and
    # continuation fallback are computed independently of generic SMC evidence.
    fvg_playbook = detect_fvg_playbook(candles)
    playbook_zones = fvg_playbook.get("zones", []) if isinstance(fvg_playbook, dict) else []
    if playbook_zones:
        fair_value_gaps = [registry.observe(z, timeframe, "FVG") if registry else z for z in playbook_zones]
    ...
    evidence = {
        ...
        "fvg_playbook": fvg_playbook,
        "ifvg_playbook": fvg_playbook.get("ifvg", []) if isinstance(fvg_playbook, dict) else [],
        "continuation_playbook": fvg_playbook.get("continuation") if isinstance(fvg_playbook, dict) else None,
